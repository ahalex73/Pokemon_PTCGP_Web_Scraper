...
def _get_pokemon_information(soup, expansion_name, image_dir, postfix):
    get_picture(soup, expansion_name, image_dir, postfix)
    #get_json_info()

    return 

def get_picture(soup, expansion_name, image_dir, postfix):
...
def get_json_info(soup, expansion_name,):
    img = soup.find(class_="card shadow resp-w")
    pokemon_name = soup.find(class_= "card-text-name").text
    ...
    if "src" in img.attrs:
        ...
        if img_response.status_code == 200:
            ...
            try:
                ...
                webp_path = os.path.join(CARD_DATA_BASE_DIR, expansion_name, f"{pokemon_name}_{postfix}.webp")
